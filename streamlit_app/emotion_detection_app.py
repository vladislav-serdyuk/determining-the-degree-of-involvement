...
class EmotionDetectionProcessor:
    """Класс для обработки видео с использованием AttentionAnalyzer"""
    ...
    def initialize_models(self, params):
        ...
        try:
            if BACKEND_AVAILABLE:
                ...
                # Основной детектор
                self.detector = AttentionAnalyzer(
                    face_detector,
                    emotion_recognizer,
...
def display_sidebar():
    ...
    with st.sidebar:
        ...
        if BACKEND_AVAILABLE:
            ...
            st.info("Убедитесь, что файл analyze_attention.py находится в текущей директории")
            ...
            with col1:
                if st.button("🚀 Начать распознавание эмоций", type="primary", width='stretch'):
                    if not BACKEND_AVAILABLE:
                        st.error(
                            "Невозможно начать обработку: Backend модуль недоступен. Убедитесь, что файл analyze_attention.py находится в текущей директории со всеми необходимыми зависимостями.")
                    else:
...
def display_error():
    ...
    st.markdown("""
    1. ✅ Убедитесь, что файл `analyze_attention.py` находится в той же директории
...
def create_webcam_section():
    ...
    if not BACKEND_AVAILABLE:
        st.warning(
            "Распознавание эмоций через веб-камеру требует модуль бэкенда. Убедитесь, что файл analyze_attention.py доступен.")
        return
        ...
        if st.session_state.get('webcam_running', False):
            ...
            if not cap.isOpened():
                ...
                if st.session_state.webcam_detector is None:
                    ...
                    st.session_state.webcam_detector = AttentionAnalyzer(
                        face_detector,
                        emotion_recognizer,
                        ear_analyzer=ear_analyzer,
                        head_pose_estimator=head_pose_estimator,
...
def main():
    ...
    with tab3:
        ...
        faqs = [
            {
                ...
                "answer": "Приложение использует класс AttentionAnalyzer, который объединяет детекцию лиц и распознавание эмоций. Оно обрабатывает каждый кадр видео в реальном времени, рисуя ограничивающие рамки и метки эмоций."
            },
            {
        ...
        issues = [
            ...
            ("Ошибки импорта",
             "Убедитесь, что файл analyze_attention.py находится в текущей директории."),
