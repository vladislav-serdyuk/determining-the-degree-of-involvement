...
6. [Научные источники](literature.md) – публикации, на которых основаны веса и модификаторы
...
Если `ear_data` или `head_pose_data = None` – соответствующий компонент получает нейтральное значение `0.5`. Поведение и влияние такого компонента на итогову метрику описано в [данном разделе](README.md#семантика-нейтрального-значения-05).
