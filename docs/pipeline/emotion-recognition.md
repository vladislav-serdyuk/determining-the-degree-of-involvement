...
Как происходит выбор устройства в пайплайне:

1. **На уровне класса** (один раз при импорте модуля) - по `settings.emotion_device` создаётся class-level `recognizer`, который используется по умолчанию всеми экземплярами.
2. **В `__init__`** - если передан **хотя бы один** из `device` или `model_name`, конструктор пересоздаёт собственный `self.recognizer`, перекрывающий class-level. Явно переданный аргумент **приоритетнее**, чем `settings.emotion_*`. Если один из двух параметров не передан, для него используется fallback на settings (`device` → `settings.emotion_device`, `model_name` → `settings.emotion_model_name`). Логика `auto` / `cuda`-fallback применяется к итоговому значению `device`. Если оба параметра `None`, блок не срабатывает - экземпляр использует class-level `recognizer` из шага 1.

Таким образом env-переменная задаёт умолчание для всего процесса, а `__init__(device=...)` точечно перекрывает её для конкретного экземпляра (например, при создании дополнительного recognizer'а для тестов на CPU).
